...
class DalolatnomaExport implements FromArray, WithHeadings, WithStyles, WithColumnWidths, WithEvents
{
    protected $id;
    protected $start;
    protected $end;
    ...
    {
        $this->kindgar = Kindgarden::where('id', $this->id)->with('age_range')->first();
        ...
        if(is_null(env('INVOICE_NUMBER'))){
            $this->invoice_number = $this->kindgar->number_of_org.'-'.$this->days->last()->month_id;
        }else{
    ...
    public function array(): array
    {
        ...
        // Kirish matni
        $intro_text = 'Бизлар қуйидаги имзо чекувчилар ' . ($this->autorser['company_name'] ?? 'ASIA BEST DISTRIBUTION SERVICE') . ' директори Б.Тажибaев бир томондан ва ' . ($this->buyurtmachi['company_name'] ?? 'Олмалик шахар 3-сони ДМТТ директори') . ' иккинчи томондан ' . $this->contract_data . 'даги шартнома асосида қуйидаги миқдорда бажарилганлиги ҳақида туздик:';
        $data[] = ['', $intro_text, ''];
        $data[] = ['', '', ''];
        ...
        foreach($this->kindgar->age_range as $age){
            ...
            $work_description = $this->buyurtmachi['address'] . ' ' . $this->kindgar->number_of_org . '-сон МТТ ' . ($age->description ?? '9-10,5 соатлик') . ' гуруҳ тарбияланувчилари учун ' . $this->days->first()->year_name . ' йил ' . $this->days->first()->day_number . '-' . $this->days->last()->day_number . ' ' . $this->days->first()->month_name . 'да аутсорсинг асосида кунига уч маҳал овқатланишни ташкил этиш бўйича:';
        ...
        $outsourcing_amount = $total_amount * (($this->costs[$age->id]->raise ?? 28.5) / 100);
        $total_amount += $outsourcing_amount;
        
        $data[] = [
            $row_number++,
            'Аутсорсинг хизмати (' . ($this->costs[$age->id]->raise ?? '28,5') . '%)',
        ...
        $data[] = [($this->autorser['company_name'] ?? 'ASIA BEST DISTRIBUTION SERVICE'), $this->kindgar->number_of_org . '-сон ДМТТ', ''];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $tableEndRow = $tableStartRow + count($this->kindgar->age_range) + 3; // +3 chunki autsorsing va jami qatorlari
