...
<div class="modal editesmodal fade" id="Modalgarden" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{route('casher.editecost')}}" method="post">
                @csrf
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{route('casher.deletecost')}}" method="POST">
                @csrf
...
<div class="container-fluid px-4">
    <form method="POST" action="{{route('casher.createcost')}}">
        @csrf
        ...
        <tbody>
            ...
            @foreach($costs as $row)
                <tr>
                    <td>{{ $row->id }}</td>
                    <td>{{ $row->cost_name }}</td>
                    <td style="text-align: end;"><i class="detete  fa fa-trash" aria-hidden="true" data-name-id="{{ $row->cost_name }}" data-delet-id="{{ $row->id }}" data-bs-toggle="modal" style="cursor: pointer; color: crimson" data-bs-target="#deleteModal"></i></td>
                </tr>
            @endforeach
        </tbody>
    </table>
